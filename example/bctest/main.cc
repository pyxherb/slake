...
#include <slake/util/bc_builder.h>
		...
		{
			{
				HostRefHolder host_ref_holder(rt->get_fixed_alloc());
				{
					...
					HostObjectRef<FnObject> f;
					HostObjectRef<RegularFnOverloadingObject> ol;

					if (!(f = FnObject::alloc(rt.get()))) {
						puts("Out of memory");
						goto end;
					}

					if (!(ol = RegularFnOverloadingObject::alloc(f.get()))) {
						puts("Out of memory");
						goto end;
					}
...
#define ABORT_IF_EMIT_FAILED(e) \
	if (!(e)) abort()
						...
						ABORT_IF_EMIT_FAILED(builder.emit_ins(
							Opcode::LTYPE, 0,
							{ InsRegType::Any, 0 }, {}, {},
							BCBuilder::Operand::new_imm(u32_as_ins_operand(0)), BCBuilder::Operand::new_imm(0)));

						ABORT_IF_EMIT_FAILED(builder.emit_ins(
							Opcode::LVAR, 0,
							{ InsRegType::Any, 1 }, { InsRegType::Any, 0 }, {},
							BCBuilder::Operand::new_imm(0), BCBuilder::Operand::new_imm(0)));

						ABORT_IF_EMIT_FAILED(builder.emit_ins(
							Opcode::LVAR, 0,
							{ InsRegType::Any, 2 }, { InsRegType::Any, 0 }, {},
							BCBuilder::Operand::new_imm(0), BCBuilder::Operand::new_imm(0)));

						auto label_alloc_result = builder.alloc_label();
						const BCBuilder::BCLabelId loop_label = label_alloc_result.value();
						builder.set_label_off(loop_label, static_cast<uint32_t>(ol->instructions.size()));

						label_alloc_result = builder.alloc_label();
						const BCBuilder::BCLabelId loop_end_label = label_alloc_result.value();

						{
							...
							ABORT_IF_EMIT_FAILED(builder.emit_ins(
								Opcode::BR, 0,
								{}, { InsRegType::Bool, 0 }, {},
								BCBuilder::Operand::new_label(loop_label), BCBuilder::Operand::new_label(loop_end_label)));
						}

						builder.set_label_off(loop_end_label, static_cast<uint32_t>(ol->instructions.size()));
					...
					Value result;
					if (auto e = rt->exec_fn(ol.get(), context.get(), nullptr, nullptr, 0, result);
						e) {
