...
std::unique_ptr<std::istream> fsModuleLocator(slake::Runtime *rt, slake::ValueRef<slake::IdRefObject> ref) {
	std::string path;
...
int main(int argc, char **argv) {
	...
	slake::ValueRef<slake::ModuleValue> mod;
	{
		...
		try {
			fs.exceptions(std::ios::failbit | std::ios::badbit | std::ios::eofbit);
			fs.open("rbtree/rbtree.slx", std::ios_base::in | std::ios_base::binary);
