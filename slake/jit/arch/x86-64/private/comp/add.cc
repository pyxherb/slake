...
template <typename T>
InternalExceptionPointer compileIntAddInstruction(
...
template <typename T>
InternalExceptionPointer compileFpAddInstruction(
	JITCompileContext &compileContext,
	const Instruction &curIns,
	const Value &lhsExpectedValue,
	const Value &rhsExpectedValue) {
		...
		switch (outputRegInfo.type.typeId) {
			case TypeId::Value: {
				switch (outputRegInfo.type.getValueTypeExData()) {
					...
					case ValueType::F64: {
						compileFpAddInstruction<double>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue);
