...
[[nodiscard]] InternalExceptionPointer compileIntAddInstruction(
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, int8_t>) {
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm8ToReg8Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int16_t>) {
			int16_t rhsData = curIns.operands[1].getI16();
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm16ToReg16Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int32_t>) {
			int32_t rhsData = curIns.operands[1].getI32();
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm32ToReg32Ins(lhsRegId, (uint8_t *)&rhsData)));
			...
			if (*((uint64_t *)&rhsData) & 0xffffffff00000000) {
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddReg64ToReg64Ins(lhsRegId, tmpGpRegId)));
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm8ToReg8Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int16_t>) {
				int16_t lhsData = curIns.operands[0].getI16();
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm16ToReg16Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int32_t>) {
				int32_t lhsData = curIns.operands[0].getI32();
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm32ToReg32Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int64_t>) {
				...
				if (*((uint64_t *)&lhsData) & 0xffffffff00000000) {
					...
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddReg64ToReg64Ins(rhsRegId, tmpGpRegId)));
...
[[nodiscard]] InternalExceptionPointer compileFpAddInstruction(
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, float>) {
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddssRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddsdRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, float>) {
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovdReg32ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddssRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovImm64ToReg64Ins(tmpGpRegId, (uint8_t *)&lhsData)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqReg64ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddsdRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
			...
			if constexpr (std::is_same_v<T, float>) {
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddssMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddssRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
				...
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddsdMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddsdRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqRegXmmToRegXmmIns(lhsXmmRegId, lhsVregState.phyReg)));
		...
		switch (outputRegInfo.type.typeId) {
			case TypeId::Value: {
				switch (outputRegInfo.type.getValueTypeExData()) {
					case ValueType::I8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<int8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<int16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<int32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<int64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<uint8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<uint16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<uint32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntAddInstruction<uint64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F32: {
						SLAKE_RETURN_IF_EXCEPT(compileFpAddInstruction<float>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F64: {
						SLAKE_RETURN_IF_EXCEPT(compileFpAddInstruction<double>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
