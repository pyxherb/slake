	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (sizeof(T) == sizeof(uint8_t)) {
			...
			if (compileContext.isRegInUse(REG_RAX)) {
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushReg(REG_RAX, savedRaxOff, savedRaxSize));
			}
		}
		...
		if constexpr (sizeof(T) == sizeof(uint8_t)) {
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitShrReg16WithImm8Ins(REG_RAX, 8)));
		} else if constexpr (sizeof(T) > sizeof(uint8_t)) {
			if (savedRaxOff != INT32_MIN) {
				SLAKE_RETURN_IF_EXCEPT(compileContext.popReg(REG_RAX, savedRaxOff, savedRaxSize));
			}
		}
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {  // The RHS is an expectable value so we can just simply add it with a register.
			...
			if (compileContext.isRegInUse(rhsRegId)) {
				int32_t off;
				size_t size;
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushReg(rhsRegId, off, size));
			}
...
[[nodiscard]] InternalExceptionPointer compileFpModInstruction(
	...
	if (compileContext.isRegInUse(rhsXmmRegId)) {
		SLAKE_RETURN_IF_EXCEPT(compileContext.pushRegXmm(rhsXmmRegId, rhsOff, rhsSize));
	}
	...
	if constexpr (std::is_same_v<T, float>) {
		...
		if (rhsVregState.saveOffset != INT32_MIN) {
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovdMemToRegXmmIns(rhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
		} else {
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqRegXmmToRegXmmIns(rhsXmmRegId, rhsVregState.phyReg)));
		}
		...
		SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitCallIns((void *)fmodfWrapper)));
		...
		if (rhsVregState.saveOffset != INT32_MIN) {
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqMemToRegXmmIns(rhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
		} else {
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqRegXmmToRegXmmIns(rhsXmmRegId, rhsVregState.phyReg)));
		}
		...
		if (padding) {
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.checkAndPushStackPointer(szDiff));
		}
		...
		SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitCallIns((void *)fmodWrapper)));

		SLAKE_RETURN_IF_EXCEPT(compileContext.restoreCallingRegs(callingRegSavingInfo));

		if (padding) {
			uint32_t szDiff = 16 - padding;
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm32ToReg64Ins(REG_RSP, (uint8_t *)&szDiff)));
	...
	if (rhsOff != INT32_MIN) {
		SLAKE_RETURN_IF_EXCEPT(compileContext.popRegXmm(rhsXmmRegId, rhsOff, rhsSize));
	}
		...
		switch (outputRegInfo.type.typeId) {
			case TypeId::Value: {
				switch (outputRegInfo.type.getValueTypeExData()) {
					case ValueType::I8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<int8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<int16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<int32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<int64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<uint8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<uint16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<uint32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntModInstruction<uint64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F32: {
						SLAKE_RETURN_IF_EXCEPT(compileFpModInstruction<float>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F64: {
						SLAKE_RETURN_IF_EXCEPT(compileFpModInstruction<double>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
