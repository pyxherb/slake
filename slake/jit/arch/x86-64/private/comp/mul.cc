...
template <typename T>
[[nodiscard]] InternalExceptionPointer compile_int_mul_instruction(
	...
	if (rhs_expected_value.value_type != ValueType::Undefined) {
		...
		if (compile_context.is_reg_in_use(tmp_reg_id)) {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul8_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul16_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul32_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul64_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
			} else if constexpr (std::is_same_v<T, uint8_t>) {
				uint8_t rhs_data = cur_ins.operands[1].get_u8();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm8_to_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 }, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul8_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
			} else if constexpr (std::is_same_v<T, uint16_t>) {
				uint16_t rhs_data = cur_ins.operands[1].get_u16();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm16_to_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 }, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul16_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
			} else if constexpr (std::is_same_v<T, uint32_t>) {
				uint32_t rhs_data = cur_ins.operands[1].get_u32();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm32_to_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 }, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul32_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
			} else if constexpr (std::is_same_v<T, uint64_t>) {
				uint64_t rhs_data = cur_ins.operands[1].get_u64();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm64_to_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 }, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul64_with_mem_ins(MemoryLocation{ REG_RBP, off, REG_MAX, 0 })));
			} else {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul8_with_reg8_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, int16_t>) {
				int16_t rhs_data = cur_ins.operands[1].get_i16();
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul16_with_reg16_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, int32_t>) {
				int32_t rhs_data = cur_ins.operands[1].get_i32();
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul32_with_reg32_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, int64_t>) {
				int64_t rhs_data = cur_ins.operands[1].get_i64();
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mul64_with_reg64_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, uint8_t>) {
				uint8_t rhs_data = cur_ins.operands[1].get_u8();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm8_to_reg8_ins(tmp_reg_id, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul8_with_reg8_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, uint16_t>) {
				uint16_t rhs_data = cur_ins.operands[1].get_u16();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm16_to_reg16_ins(tmp_reg_id, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul16_with_reg16_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, uint32_t>) {
				uint32_t rhs_data = cur_ins.operands[1].get_u32();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm32_to_reg32_ins(tmp_reg_id, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul32_with_reg32_ins(tmp_reg_id)));
			} else if constexpr (std::is_same_v<T, uint64_t>) {
				uint64_t rhs_data = cur_ins.operands[1].get_u64();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm64_to_reg64_ins(tmp_reg_id, (uint8_t *)&rhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_imul64_with_reg64_ins(tmp_reg_id)));
			} else {
				static_assert(!std::is_same_v<T, T>, "Invalid operand type");
			}
		...
		if (lhs_expected_value.value_type != ValueType::Undefined) {  // The RHS is an expectable value so we can just simply add it with a register.
			...
			if (rhs_vreg_state.save_offset != INT32_MIN) {
				if constexpr (std::is_same_v<T, int8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul8_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul16_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul32_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul64_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, uint8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul8_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, uint16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul16_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, uint32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul32_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, uint64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul64_with_mem_ins(MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else {
					static_assert(!std::is_same_v<T, T>, "Invalid operand type");
				}
			} else {
				if constexpr (std::is_same_v<T, int8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul8_with_reg8_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul16_with_reg16_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul32_with_reg32_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_imul64_with_reg64_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, uint8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul8_with_reg8_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, uint16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul16_with_reg16_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, uint32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul32_with_reg32_ins(rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, uint64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_mul64_with_reg64_ins(rhs_vreg_state.phy_reg)));
				} else {
					static_assert(!std::is_same_v<T, T>, "Invalid operand type");
				}
			}
...
template <typename T>
[[nodiscard]] InternalExceptionPointer compile_fp_mul_instruction(
	JITCompileContext &compile_context,
	const Instruction &cur_ins,
	const Value &lhs_expected_value,
	const Value &rhs_expected_value) noexcept {
	...
	if (rhs_expected_value.value_type != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, float>) {
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulss_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, tmp_xmm_reg_id)));
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulsd_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, tmp_xmm_reg_id)));
		...
		if (lhs_expected_value.value_type != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, float>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movd_reg32_to_reg_xmm_ins(tmp_xmm_reg_id, tmp_gp_reg_id)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulss_reg_xmm_to_reg_xmm_ins(rhs_xmm_reg_id, tmp_xmm_reg_id)));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movq_reg64_to_reg_xmm_ins(tmp_xmm_reg_id, tmp_gp_reg_id)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulsd_reg_xmm_to_reg_xmm_ins(rhs_xmm_reg_id, tmp_xmm_reg_id)));
			...
			if constexpr (std::is_same_v<T, float>) {
				if (lhs_vreg_state.save_offset != INT32_MIN) {
					...
					if (rhs_vreg_state.save_offset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulss_mem_to_reg_xmm_ins(lhs_xmm_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
					} else {
						RegisterId rhs_xmm_reg_id = rhs_vreg_state.phy_reg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulss_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, rhs_xmm_reg_id)));
					}
				} else {
				...
				if (lhs_vreg_state.save_offset != INT32_MIN) {
					...
					if (rhs_vreg_state.save_offset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulsd_mem_to_reg_xmm_ins(lhs_xmm_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
					} else {
						RegisterId rhs_xmm_reg_id = rhs_vreg_state.phy_reg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mulsd_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, rhs_xmm_reg_id)));
					}
				} else {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movq_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, lhs_vreg_state.phy_reg)));
...
InternalExceptionPointer slake::jit::x86_64::compile_mul_instruction(
	JITCompileContext &compile_context,
	...
	switch (output_reg_info.type.type_id) {
		case TypeId::I8: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<int8_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I16: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<int16_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<int32_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<int64_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U8: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<uint8_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U16: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<uint16_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<uint32_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_mul_instruction<uint64_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::F32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_fp_mul_instruction<float>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::F64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_fp_mul_instruction<double>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
