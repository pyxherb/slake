...
template <typename T>
[[nodiscard]] InternalExceptionPointer compile_int_sub_instruction(
	...
	if (rhs_expected_value.value_type != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, int8_t>) {
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm8_to_reg8_ins(lhs_reg_id, (uint8_t *)&rhs_data)));
		} else if constexpr (std::is_same_v<T, int16_t>) {
			int16_t rhs_data = cur_ins.operands[1].get_i16();
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm16_to_reg16_ins(lhs_reg_id, (uint8_t *)&rhs_data)));
		} else if constexpr (std::is_same_v<T, int32_t>) {
			int32_t rhs_data = cur_ins.operands[1].get_i32();
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm32_to_reg32_ins(lhs_reg_id, (uint8_t *)&rhs_data)));
			...
			if (*((uint64_t *)&rhs_data) & 0xffffffff00000000) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_reg64_to_reg64_ins(lhs_reg_id, tmp_gp_reg_id)));
		...
		if (lhs_expected_value.value_type != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm8_to_reg8_ins(rhs_reg_id, (uint8_t *)&lhs_data)));
			} else if constexpr (std::is_same_v<T, int16_t>) {
				int16_t lhs_data = cur_ins.operands[1].get_i16();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm16_to_reg16_ins(rhs_reg_id, (uint8_t *)&lhs_data)));
			} else if constexpr (std::is_same_v<T, int32_t>) {
				int32_t lhs_data = cur_ins.operands[1].get_i32();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_imm32_to_reg32_ins(rhs_reg_id, (uint8_t *)&lhs_data)));
			} else if constexpr (std::is_same_v<T, int64_t>) {
				...
				if (*((uint64_t *)&lhs_data) & 0xffffffff00000000) {
					...
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_sub_reg64_to_reg64_ins(rhs_reg_id, tmp_gp_reg_id)));
			...
			if (rhs_vreg_state.save_offset != INT32_MIN) {
				if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_mem_to_reg8_ins(lhs_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int16_t> || std::is_same_v<T, uint16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_mem_to_reg16_ins(lhs_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, uint32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_mem_to_reg32_ins(lhs_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_mem_to_reg64_ins(lhs_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
				} else {
					static_assert(!std::is_same_v<T, T>, "Invalid operand type");
				}
			} else {
				if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_reg8_to_reg8_ins(lhs_reg_id, rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int16_t> || std::is_same_v<T, uint16_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_reg16_to_reg16_ins(lhs_reg_id, rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, uint32_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_reg32_to_reg32_ins(lhs_reg_id, rhs_vreg_state.phy_reg)));
				} else if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(
																	emit_sub_reg64_to_reg64_ins(lhs_reg_id, rhs_vreg_state.phy_reg)));
				} else {
					static_assert(!std::is_same_v<T, T>, "Invalid operand type");
				}
			}
...
template <typename T>
[[nodiscard]] InternalExceptionPointer compile_fp_sub_instruction(
	JITCompileContext &compile_context,
	const Instruction &cur_ins,
	const Value &lhs_expected_value,
	const Value &rhs_expected_value) noexcept {
	...
	if (rhs_expected_value.value_type != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, float>) {
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subss_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, tmp_xmm_reg_id)));
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subsd_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, tmp_xmm_reg_id)));
		...
		if (lhs_expected_value.value_type != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, float>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movd_reg32_to_reg_xmm_ins(tmp_xmm_reg_id, tmp_gp_reg_id)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subss_reg_xmm_to_reg_xmm_ins(rhs_xmm_reg_id, tmp_xmm_reg_id)));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_mov_imm64_to_reg64_ins(tmp_gp_reg_id, (uint8_t *)&lhs_data)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movq_reg64_to_reg_xmm_ins(tmp_xmm_reg_id, tmp_gp_reg_id)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subsd_reg_xmm_to_reg_xmm_ins(rhs_xmm_reg_id, tmp_xmm_reg_id)));
			...
			if constexpr (std::is_same_v<T, float>) {
				if (lhs_vreg_state.save_offset != INT32_MIN) {
					...
					if (rhs_vreg_state.save_offset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subss_mem_to_reg_xmm_ins(lhs_xmm_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
					} else {
						RegisterId rhs_xmm_reg_id = rhs_vreg_state.phy_reg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subss_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, rhs_xmm_reg_id)));
					}
				} else {
				...
				if (lhs_vreg_state.save_offset != INT32_MIN) {
					...
					if (rhs_vreg_state.save_offset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subsd_mem_to_reg_xmm_ins(lhs_xmm_reg_id, MemoryLocation{ REG_RBP, rhs_vreg_state.save_offset, REG_MAX, 0 })));
					} else {
						RegisterId rhs_xmm_reg_id = rhs_vreg_state.phy_reg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_subsd_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, rhs_xmm_reg_id)));
					}
				} else {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_context.push_ins(emit_movq_reg_xmm_to_reg_xmm_ins(lhs_xmm_reg_id, lhs_vreg_state.phy_reg)));
...
InternalExceptionPointer slake::jit::x86_64::compile_sub_instruction(
	JITCompileContext &compile_context,
	...
	switch (output_reg_info.type.type_id) {
		case TypeId::I8: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<int8_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I16: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<int16_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<int32_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::I64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<int64_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U8: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<uint8_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U16: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<uint16_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<uint32_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::U64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_int_sub_instruction<uint64_t>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::F32: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_fp_sub_instruction<float>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
		...
		case TypeId::F64: {
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compile_fp_sub_instruction<double>(
															compile_context,
															cur_ins,
															lhs_expected_value,
															rhs_expected_value));
