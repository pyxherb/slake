...
template <typename T>
[[nodiscard]] InternalExceptionPointer compileIntSubInstruction(
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, int8_t>) {
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm8ToReg8Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int16_t>) {
			int16_t rhsData = curIns.operands[1].getI16();
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm16ToReg16Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int32_t>) {
			int32_t rhsData = curIns.operands[1].getI32();
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm32ToReg32Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int64_t>) {
			int64_t rhsData = curIns.operands[1].getI64();

			if (*((uint64_t *)&rhsData) & 0xffffffff00000000) {
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm32ToReg64Ins(lhsRegId, (uint8_t *)&rhsData)));
			} else {
				...
				if (compileContext.isRegInUse(tmpGpRegId)) {
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushReg(tmpGpRegId, tmpGpRegSavedOff, tmpGpRegSavedSize));
				}

				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovImm64ToReg64Ins(tmpGpRegId, (uint8_t *)&rhsData)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubReg64ToReg64Ins(lhsRegId, tmpGpRegId)));

				if (tmpGpRegSavedOff != INT32_MIN) {
					SLAKE_RETURN_IF_EXCEPT(compileContext.popReg(tmpGpRegId, tmpGpRegSavedOff, tmpGpRegSavedSize));
				}
			}
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm8ToReg8Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int16_t>) {
				int16_t lhsData = curIns.operands[1].getI16();
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm16ToReg16Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int32_t>) {
				int32_t lhsData = curIns.operands[1].getI32();
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubImm32ToReg32Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int64_t>) {
				int64_t lhsData = curIns.operands[1].getI64();

				if (*((uint64_t *)&lhsData) & 0xffffffff00000000) {
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitAddImm32ToReg64Ins(rhsRegId, (uint8_t *)&lhsData)));
				} else {
					RegisterId tmpGpRegId = compileContext.allocGpReg();
					...
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubReg64ToReg64Ins(rhsRegId, tmpGpRegId)));
...
template <typename T>
[[nodiscard]] InternalExceptionPointer compileFpSubInstruction(
	JITCompileContext &compileContext,
	const Instruction &curIns,
	const Value &lhsExpectedValue,
	const Value &rhsExpectedValue) noexcept {
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, float>) {
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubssRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
			...
			SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, float>) {
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovdReg32ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubssRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
				...
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovImm64ToReg64Ins(tmpGpRegId, (uint8_t *)&lhsData)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqReg64ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
			...
			if constexpr (std::is_same_v<T, float>) {
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubssMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubssRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
				...
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubsdMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
					SLAKE_RETURN_IF_EXCEPT(compileContext.pushIns(emitMovqRegXmmToRegXmmIns(lhsXmmRegId, lhsVregState.phyReg)));
		...
		switch (outputRegInfo.type.typeId) {
			case TypeId::Value: {
				switch (outputRegInfo.type.getValueTypeExData()) {
					case ValueType::I8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<int8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<int16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<int32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::I64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<int64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U8: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<uint8_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U16: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<uint16_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U32: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<uint32_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::U64: {
						SLAKE_RETURN_IF_EXCEPT(compileIntSubInstruction<uint64_t>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F32: {
						SLAKE_RETURN_IF_EXCEPT(compileFpSubInstruction<float>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
					...
					case ValueType::F64: {
						SLAKE_RETURN_IF_EXCEPT(compileFpSubInstruction<double>(
							compileContext,
							curIns,
							lhsExpectedValue,
							rhsExpectedValue));
