...
[[nodiscard]] InternalExceptionPointer compileIntSubInstruction(
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, int8_t>) {
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm8ToReg8Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int16_t>) {
			int16_t rhsData = curIns.operands[1].getI16();
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm16ToReg16Ins(lhsRegId, (uint8_t *)&rhsData)));
		} else if constexpr (std::is_same_v<T, int32_t>) {
			int32_t rhsData = curIns.operands[1].getI32();
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm32ToReg32Ins(lhsRegId, (uint8_t *)&rhsData)));
			...
			if (*((uint64_t *)&rhsData) & 0xffffffff00000000) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubReg64ToReg64Ins(lhsRegId, tmpGpRegId)));
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, int8_t>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm8ToReg8Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int16_t>) {
				int16_t lhsData = curIns.operands[1].getI16();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm16ToReg16Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int32_t>) {
				int32_t lhsData = curIns.operands[1].getI32();
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubImm32ToReg32Ins(rhsRegId, (uint8_t *)&lhsData)));
			} else if constexpr (std::is_same_v<T, int64_t>) {
				...
				if (*((uint64_t *)&lhsData) & 0xffffffff00000000) {
					...
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubReg64ToReg64Ins(rhsRegId, tmpGpRegId)));
...
[[nodiscard]] InternalExceptionPointer compileFpSubInstruction(
	...
	if (rhsExpectedValue.valueType != ValueType::Undefined) {
		...
		if constexpr (std::is_same_v<T, float>) {
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubssRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
			...
			SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(lhsXmmRegId, tmpXmmRegId)));
		...
		if (lhsExpectedValue.valueType != ValueType::Undefined) {
			...
			if constexpr (std::is_same_v<T, float>) {
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitMovdReg32ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubssRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
				...
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitMovImm64ToReg64Ins(tmpGpRegId, (uint8_t *)&lhsData)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitMovqReg64ToRegXmmIns(tmpXmmRegId, tmpGpRegId)));
				SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(rhsXmmRegId, tmpXmmRegId)));
			...
			if constexpr (std::is_same_v<T, float>) {
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubssMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubssRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
				...
				if (lhsVregState.saveOffset != INT32_MIN) {
					...
					if (rhsVregState.saveOffset != INT32_MIN) {
						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubsdMemToRegXmmIns(lhsXmmRegId, MemoryLocation{ REG_RBP, rhsVregState.saveOffset, REG_MAX, 0 })));
					} else {
						RegisterId rhsXmmRegId = rhsVregState.phyReg;

						SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitSubsdRegXmmToRegXmmIns(lhsXmmRegId, rhsXmmRegId)));
					}
				} else {
					SLAKE_RETURN_IF_EXCEPT_WITH_LVAR(exception, compileContext.pushIns(emitMovqRegXmmToRegXmmIns(lhsXmmRegId, lhsVregState.phyReg)));
