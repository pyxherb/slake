...
InternalExceptionPointer compileInstruction(
	...
	case Opcode::LOAD: {
		...
		{
			IdRefObject *refObj = (IdRefObject *)curIns.operands[0].getEntityRef().asInstance.instanceObject;
	...
	case Opcode::RLOAD: {
		...
		{
			IdRefObject *refObj = (IdRefObject *)curIns.operands[1].getEntityRef().asInstance.instanceObject;
