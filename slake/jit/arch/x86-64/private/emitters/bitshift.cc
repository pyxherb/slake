...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg8WithImm8Ins(RegisterId registerId, uint8_t times) {
	switch (registerId) {
		...
		case REG_RBX: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, 0xd0, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, 0xc0, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_RDI: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0xd0, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0xc0, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_R15: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xd0, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xc0, MODRM_BYTE(0b11, 5, (registerId - REG_R8)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg16WithImm8Ins(RegisterId registerId, uint8_t times) {
	switch (registerId) {
		...
		case REG_RBX: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, 0x66, 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, 0x66, 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_RDI: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0x66, 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0x66, 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_R15: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0x66, 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0x66, 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg32WithImm8Ins(RegisterId registerId, uint8_t times) {
	switch (registerId) {
		...
		case REG_RBX: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_RDI: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_R15: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg64WithImm8Ins(RegisterId registerId, uint8_t times) {
	switch (registerId) {
		...
		case REG_RBX: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 0), 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 0), 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
		...
		case REG_R15: {
			if (times == 1) {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 1), 0xd1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
				return emitRawIns(sizeof(insBuf), insBuf);
			} else {
				DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 1), 0xc1, MODRM_BYTE(0b11, 5, (registerId - REG_R8)),
					times);
				return emitRawIns(sizeof(insBuf), insBuf);
			}
		}
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg8WithClIns(RegisterId registerId) {
	switch (registerId) {
		...
		case REG_RDX:
		case REG_RBX: {
			DEF_INS_BUFFER(insBuf, 0xd2, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		case REG_RSP:
		case REG_RBP:
		case REG_RSI:
		case REG_RDI: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 0), 0xd2, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		case REG_R8:
		case REG_R9:
		...
		case REG_R14:
		case REG_R15: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xd2, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		default:
			throw std::logic_error("Invalid register ID");
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg16WithClIns(RegisterId registerId) {
	switch (registerId) {
		...
		case REG_RDX:
		case REG_RBX: {
			DEF_INS_BUFFER(insBuf, 0x66, 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		case REG_RSP:
		case REG_RBP:
		...
		case REG_R14:
		case REG_R15: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0x66, 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		default:
			throw std::logic_error("Invalid register ID");
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg32WithClIns(RegisterId registerId) {
	switch (registerId) {
		...
		case REG_RDX:
		case REG_RBX: {
			DEF_INS_BUFFER(insBuf, 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		case REG_RSP:
		case REG_RBP:
		...
		case REG_R14:
		case REG_R15: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(0, 0, 0, 1), 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		default:
			throw std::logic_error("Invalid register ID");
...
SLAKE_API DiscreteInstruction slake::jit::x86_64::emitShrReg64WithClIns(RegisterId registerId) {
	switch (registerId) {
		...
		case REG_RDX:
		case REG_RBX: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 0), 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_RAX)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		case REG_RSP:
		case REG_RBP:
		...
		case REG_R14:
		case REG_R15: {
			DEF_INS_BUFFER(insBuf, REX_PREFIX(1, 0, 0, 1), 0xd3, MODRM_BYTE(0b11, 5, (registerId - REG_R8)));
			return emitRawIns(sizeof(insBuf), insBuf);
		}
		default:
			throw std::logic_error("Invalid register ID");
