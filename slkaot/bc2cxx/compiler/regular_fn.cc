...
void BC2CXX::recompileRegularFnOverloading(CompileContext &compileContext, std::shared_ptr<cxxast::Fn> fnOverloading) {
	...
	for (size_t i = 0; i < fo->paramTypes.size(); ++i) {
		...
		fnOverloading->signature.paramTypes.push_back(compileParamType(compileContext, fo->paramTypes.at(i)));
	}
