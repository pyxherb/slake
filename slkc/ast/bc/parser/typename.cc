...
SLKC_API peff::Option<SyntaxError> BCParser::parseGenericArg(AstNodePtr<AstNode>& argOut) {
	...
	switch (t->tokenId) {
		...
		case TokenId::StringLiteral: {
			...
			if((syntaxError = parseComptimeExpr(e)))
				return syntaxError;
...
SLKC_API peff::Option<SyntaxError> BCParser::parseTypeName(AstNodePtr<TypeNameNode> &typeNameOut, bool withCircumfixes) {
	peff::Option<SyntaxError> syntaxError;
	Token *t = peekToken();
	...
	switch (t->tokenId) {
		...
		case TokenId::SIMDTypeName: {
			...
			if (auto e = parseComptimeExpr(tn->width); e)
				return e;
		...
		case TokenId::Id: {
			...
			if (!(tn = makeAstNode<BCCustomTypeNameNode>(
					  resourceAllocator.get(),
					  resourceAllocator.get(),
					  document)))
				return genOutOfMemorySyntaxError();
