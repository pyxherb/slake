...
void Compiler::compileExpr(CompileContext *compileContext, std::shared_ptr<ExprNode> expr) {
	...
	if (!compileContext->curTopLevelContext.curMajorContext.curMinorContext.dryRun) {
		if (auto ce = evalConstExpr(compileContext, expr); ce) {
			...
			compileContext->_insertIns(Opcode::MOV, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest, { ce });
			return;
		}
	}
	...
	case ExprType::Ternary: {
		...
		if (isSameType(compileContext, trueBranchType, resultType)) {
			...
			compileExpr(compileContext,
				...
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose,
				std::make_shared<RegRefNode>(resultRegIndex));
		} else
			compileExpr(compileContext, e->x, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose, std::make_shared<RegRefNode>(resultRegIndex));
		...
		if (isSameType(compileContext, falseBranchType, resultType)) {
			...
			compileExpr(compileContext, e->y, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose, std::make_shared<RegRefNode>(resultRegIndex));
	...
	case ExprType::Match: {
		...
		for (auto i : e->cases) {
			...
			compileExpr(compileContext, i.second, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest);
	...
	case ExprType::Call: {
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.isArgTypesSet = true;
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.argTypes = {};
		...
		for (auto &i : e->args) {
			...
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.argTypes.push_back(type);
		}
		...
		auto returnType = compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetReturnType;
		if (!returnType)
			throw FatalCompilationError(
				Message(
		...
		for (size_t i = 0; i < e->args.size(); ++i) {
			...
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.curCorrespondingArgIndex = i;
			if (i < compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams.size()) {
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.curCorrespondingParam = compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams[i];
				if (!i) {
					...
					updateTokenInfo(e->idxLParentheseToken, [this, &i, &compileContext](TokenInfo &info) {
						info.semanticInfo.correspondingParam = compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams[i];
					});
				} else {
			...
			if (i < compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams.size()) {
				if (isLValueType(compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams[i]->type))
		...
		switch (compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose) {
		case EvalPurpose::LValue: {
			...
			if (compileContext->curTopLevelContext.curMajorContext.curMinorContext.isLastCallTargetStatic)
				compileContext->_insertIns(
					Opcode::CALL,
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = returnType;
	...
	case ExprType::New: {
			...
			case NodeType::Interface: {
				...
				if (auto it = n->scope->members.find("new"); it != n->scope->members.end()) {
					...
					compileContext->_insertIns(
						Opcode::CTORCALL,
						{},
						{ std::make_shared<RegRefNode>(ctorIndex), compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest });
				} else {
					compileContext->_insertIns(
						Opcode::NEW,
						compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest,
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = e->type;
	...
	case ExprType::Cast: {
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.expectedType = e->targetType;

		if (auto ce = evalConstExpr(compileContext, e->target); ce) {
			if (isTypeNamesConvertible(compileContext, evalExprType(compileContext, ce), e->targetType)) {
				compileContext->_insertIns(Opcode::CAST, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest, { e->targetType, ce });
			} else {
			...
			if (isTypeNamesConvertible(compileContext, originalType, e->targetType)) {
				...
				compileExpr(compileContext, e->target, EvalPurpose::RValue, std::make_shared<RegRefNode>(tmpRegIndex));
				compileContext->_insertIns(Opcode::CAST, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest, { e->targetType, std::make_shared<RegRefNode>(tmpRegIndex) });
			} else {
				throw FatalCompilationError({ tokenRangeToSourceLocation(e->tokenRange), MessageType::Error, "Invalid type conversion" });
			}
		}

		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = e->targetType;
	...
	case ExprType::IdRef: {
		...
		auto determineOverloadingRegistry = [this, expr, &resolvedParts, &compileContext, &isStatic](std::shared_ptr<FnNode> x, const std::deque<std::shared_ptr<TypeNameNode>> &genericArgs) -> std::shared_ptr<FnOverloadingNode> {
			if ((resolvedParts.size() > 2) || (compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose != EvalPurpose::Call)) {
				//
			...
			//
			{
				...
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetParams = overloadings[0]->params;
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.hasVarArgs = overloadings[0]->isVaridic();

				compileContext->curTopLevelContext.curMajorContext.curMinorContext.lastCallTargetReturnType =
					overloadings[0]->returnType
						? overloadings[0]->returnType
						: std::make_shared<AnyTypeNameNode>(SIZE_MAX);
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.isLastCallTargetStatic = overloadings[0]->access & ACCESS_STATIC;
			...
			for (size_t i = 1; i < resolvedParts.size(); ++i) {
				...
				case NodeType::Var: {
					...
					if ((i + 1 < resolvedParts.size())) {
						...
						compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = varNode->type->duplicate<TypeNameNode>();
						compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType->isRef = true;
					}
				...
				case NodeType::Fn: {
					...
					if (!compileContext->curTopLevelContext.curMajorContext.curMinorContext.isLastCallTargetStatic) {
						compileContext->_insertIns(
							Opcode::MOV,
							compileContext->curTopLevelContext.curMajorContext.curMinorContext.thisDest,
							{ std::make_shared<RegRefNode>(tmpRegIndex) });
					}
...

					{
						...
						compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType =
					...
					if (overloading->isVirtual) {
						...
						resolvedParts[i].first->entries.back().paramTypes = compileContext->curTopLevelContext.curMajorContext.curMinorContext.argTypes;
						resolvedParts[i].first->entries.back().hasVarArg = compileContext->curTopLevelContext.curMajorContext.curMinorContext.hasVarArgs;
		...
		case NodeType::LocalVar: {
			...
			if (resolvedParts.size() > 1) {
				...
				unwrap = compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose != EvalPurpose::LValue;
			}
			...
			if (resolvedParts.size() == 1) {
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = localVarNode->type->duplicate<TypeNameNode>();
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType->isRef = true;
			}

			uint32_t resultRegIndex = loadRest();
			if (resolvedParts.back().second->getNodeType() == NodeType::Var) {
				if (compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalPurpose == EvalPurpose::RValue) {
					uint32_t newResultRegIndex = compileContext->allocReg();
					compileContext->_insertIns(
						Opcode::LVALUE,
						std::make_shared<RegRefNode>(newResultRegIndex),
			...
			if (compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest)
				compileContext->_insertIns(
					Opcode::MOV,
					compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest,
					{ std::make_shared<RegRefNode>(resultRegIndex) });
		...
		case NodeType::Param: {
			...
			if (resolvedParts.size() == 1) {
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = paramNode->type->duplicate<TypeNameNode>();
				compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType->isRef = true;
			}
		...
		case NodeType::Module: {
			...
			case NodeType::ThisRef: {
				auto owner = compileContext->curTopLevelContext.curMajorContext.curMinorContext.curScope->owner;

				switch (owner->getNodeType()) {
				case NodeType::Class:
				case NodeType::Interface: {
					compileContext->_insertIns(Opcode::LTHIS, std::make_shared<RegRefNode>(tmpRegIndex), {});
					compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = compileContext->curTopLevelContext.curMajorContext.thisType;
					break;
				}
				default:
					throw FatalCompilationError(
	...
	case ExprType::VarArg: {
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<ArrayTypeNameNode>(std::make_shared<AnyTypeNameNode>(SIZE_MAX));
	...
	case ExprType::Array: {
		...
		if ((!compileContext->curTopLevelContext.curMajorContext.curMinorContext.expectedType) ||
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.expectedType->getTypeId() != TypeId::Array)
			throw FatalCompilationError(
				Message(
					tokenRangeToSourceLocation(e->tokenRange),
					MessageType::Error,
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = type;
	...
	case ExprType::Bool: {
		...
		compileContext->_insertIns(Opcode::MOV, compileContext->curTopLevelContext.curMajorContext.curMinorContext.evalDest, { expr });

		switch (expr->getExprType()) {
		case ExprType::I8:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<I8TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::I16:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<I16TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::I32:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<I32TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::I64:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<I64TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::U8:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<U8TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::U16:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<U16TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::U32:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<U32TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::U64:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<U64TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::F32:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<F32TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::F64:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<F64TypeNameNode>(SIZE_MAX);
			break;
		case ExprType::String:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<StringTypeNameNode>(SIZE_MAX);
			break;
		case ExprType::Bool:
			compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<BoolTypeNameNode>(SIZE_MAX);
			break;
		default:;
		}
		break;
	...
	case ExprType::Null:
		...
		compileContext->curTopLevelContext.curMajorContext.curMinorContext.evaluatedType = std::make_shared<AnyTypeNameNode>(SIZE_MAX);
		break;
	case ExprType::Bad:
		break;
	default:
